        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 flex flex-col sm:justify-center items-center">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <a href="@isset($softbrick->token) webcal://{{ config('app.url') . '/storage/calendar/feed/' . $softbrick->uuid . '.ics' }} @endisset" class="bg-indigo-500 text-white font-bold py-2 px-4 rounded inline-flex items-center @if(!isset($softbrick->token)) opacity-50 cursor-not-allowed @endif">
                    ...
                    <a href="@isset($softbrick->token) http://www.google.com/calendar/render?cid={{ Request::fullUrl() . '/storage/calendar/feed/' . $softbrick->uuid . '.ics' }} @endisset" class="bg-indigo-500 text-white font-bold py-2 px-4 rounded inline-flex items-center @if(!isset($softbrick->token)) opacity-50 cursor-not-allowed @endif" target="_blank">
                    ...
                    <a href="@isset($softbrick->token) http://calendar.live.com/calendar/calendar.aspx?rru=addsubscription&name=Calendar&url=webcal://{{ Request::fullUrl() . '/storage/calendar/feed/' . $softbrick->uuid . '.ics' }} @endisset" class="bg-indigo-500 text-white font-bold py-2 px-4 rounded inline-flex items-center @if(!isset($softbrick->token)) opacity-50 cursor-not-allowed @endif" target="_blank">
